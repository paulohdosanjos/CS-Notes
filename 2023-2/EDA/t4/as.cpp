...
class As {
  ...
  public:
    ...
    { 
      ...
      for(int i = 0; i < num_segs; i++) _Insert(root, S, i);
    }
  ...
  private:
    ...
    // Insere segmento Si nas listas de segmentos dos nós da árvore enraizada em r
    void _Insert(Node* r, std::vector<std::pair<int,int>> S, int i)
    {
      ...
      if(!(Disjoint(S[i].first, S[i].second, r->left->l, r->left->left_open, r->left->r, r->left->right_open))) _Insert(r->left, S, i);
      // Mesma coisa para o filho direito
      if(!(Disjoint(S[i].first, S[i].second, r->right->l, r->right->left_open, r->right->r, r->right->right_open))) _Insert(r->right, S, i);
    }
    ...
    std::pair<Node*, int> _Build(Node* r, int height, int i, std::vector<int> e)
    {
      if(height != 0) // Nó interno ou folha de nível "superior"
      { 
        r = new Node();

        std::pair<Node*, int> pl = _Build(r->left, height - 1, i, e);
        r->left = pl.first;

        std::pair<Node*, int> pr = _Build(r->right, height - 1, i + pl.second, e);
        r->right = pr.first;
      ...
      else // Folha do último nível
      {
        ...
        // Limite já estourado. Não faz nada nesse nível
